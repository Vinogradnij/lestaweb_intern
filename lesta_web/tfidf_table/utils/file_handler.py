...
def compute_tfidf(files: list[UploadedFile]) -> list[dict]:
            ...
            for line in file:
                ...
                for word in line.split():
                    ...
                    if normal_word in self._words:
                        self._words[normal_word].add(file.name)
                    else:
                        self._words[normal_word] = Word(normal_word, file.name)
                    all_words_in_file += 1
            self._count_all_words_in_file[file.name] = all_words_in_file
